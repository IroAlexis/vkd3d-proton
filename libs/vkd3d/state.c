...
static uint32_t vkd3d_bindless_build_mutable_type_list(struct d3d12_device *device, VkDescriptorType *list)
{
    uint32_t count = 0;
    if ((device->bindless_state.flags & VKD3D_BINDLESS_CBV) &&
        !(device->bindless_state.flags & VKD3D_BINDLESS_CBV_AS_SSBO))
        list[count++] = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;

    if (device->bindless_state.flags & (VKD3D_BINDLESS_CBV_AS_SSBO | VKD3D_BINDLESS_RAW_SSBO))
        list[count++] = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    if (device->bindless_state.flags & VKD3D_BINDLESS_UAV)
    {
        list[count++] = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        list[count++] = VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER;
    }

    if (device->bindless_state.flags & VKD3D_BINDLESS_SRV)
    {
        ...
        struct d3d12_device *device, uint32_t flags, VkDescriptorType vk_descriptor_type)
{
    ...
    if (vk_descriptor_type == VK_DESCRIPTOR_TYPE_MUTABLE_VALVE)
    {
        ...
        mutable_info.descriptorTypeListCount = set_info->binding_index + 1;
        ...
        mutable_descriptor_list[set_info->binding_index].descriptorTypeCount =
                vkd3d_bindless_build_mutable_type_list(device, mutable_descriptor_types);
...
static bool vkd3d_bindless_supports_mutable_type(struct d3d12_device *device)
{
    ...
    const VkDescriptorBindingFlagsEXT binding_flag =
    ...
    mutable_info.descriptorTypeListCount = 1;

    mutable_list.descriptorTypeCount = vkd3d_bindless_build_mutable_type_list(device, descriptor_types);
        ...
        struct d3d12_device *device)
{
    ...
    if (vkd3d_bindless_supports_mutable_type(device))
    {
        ...
        bindless_state->flags |= VKD3D_BINDLESS_MUTABLE_TYPE;
